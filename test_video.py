...
# Basic options
parser.add_argument("--content_video", type=str, help="File path to the content video")
parser.add_argument(
    "--style_path", type=str, help="File path to the style video or single image"
)
...
parser.add_argument(
    "--save_ext", default=".mp4", help="The extension name of the output video"
)
parser.add_argument(
    "--output", type=str, default="output", help="Directory to save the output image(s)"
)
...
if style_path.suffix in [".mp4", ".mpg", ".avi"]:
    ...
    assert (
        style_video_length == content_video_length
    ), "Content video and style video has different number of frames"
    ...
    while True:
        ...
        output = cv2.resize(
            output, (output_width, output_height), interpolation=cv2.INTER_CUBIC
        )
